...
def parse_s3_object_url(url_string):
    ...
    if re.fullmatch(S3_DOMAIN_PATTERN, url.netloc):
        ...
        s3_folder = url.path.split('/')[1] # get the folder
        folder_len = len(s3_folder)
        if url.path[folder_len:0] == "/":
            folder_len = folder_len +1
        else:
            folder_len = folder_len +2
        s3_source_object = url.path[folder_len:]
    object_key = urllib.parse.unquote_plus(s3_object)
    object_key_folder = urllib.parse.unquote_plus(s3_folder) #replace %xx and plus signs with spaces, as required for unquoting HTML form values.
    object_key_source_object = urllib.parse.unquote_plus(s3_source_object) #replace %xx and plus signs with spaces, as required for unquoting HTML form values.

    return bucket, object_key, object_key_folder, object_key_source_object
...
def copy_object(source_bucket, dest_bucket, dest_folder, source_key, dest_key, tags, metadata, acl=None):
    params = {
    ...
    destination_key = dest_folder + "/" + dest_key
    
    s3.copy(copy_source, dest_bucket, destination_key, params, Config=transfer_config)
...
def lambda_handler(event, context):
    ...
    quarantine_folder = os.environ.get('QUARANTINEFOLDER')
    promote_folder = os.environ.get('PROMOTEFOLDER')
    ...
    for record in event['Records']:
        ...
        src_bucket, object_key, object_key_folder, object_key_source_object = parse_s3_object_url(message['file_url'])
        ...
        dst_folder = quarantine_folder if findings else promote_folder
        ...
        if not dst_folder:
            print(f'Skip: No folder specified for {operation}')
            continue
        ...
        copy_object(
            ...
            dest_key=object_key_source_object,
            tags=tags,
            metadata=metadata,
            acl=acl,
        )
